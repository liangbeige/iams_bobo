<template>
  <div class="box partner-node-collect">
    <div class="header">
      <div class="title">库房档案柜数量分布Top5</div>
      <svg-icon name="more" class="more" @click="handleMoreClick" />
    </div>
    <el-row :gutter="20" type="flex" align="middle" class="body">
      <el-col :span="17">
        <partner-node-collect-pie-chart :chart-option="props.nodeData?.seriesData" />
      </el-col>
      <el-col :span="7">
        <div class="collect">
          <div class="count">
            {{ props.nodeData?.cabinetCount }}
          </div>
          <div class="name">档案柜</div>
          <div class="count count2">
            {{ props.nodeData?.repositoryCount }}
          </div>
          <div class="name">库房</div>
        </div>
      </el-col>
    </el-row>
  </div>
