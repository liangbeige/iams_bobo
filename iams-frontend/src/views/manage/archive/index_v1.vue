<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="archiveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文件(档)数量" align="center" prop="documentCount" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" @click="gotoDetail(scope.row)" v-hasPermi="['manage:archive:query']">查看</el-button>
          <el-button link type="primary" @click="handleUpdate(scope.row)" v-hasPermi="['manage:archive:edit']">修改</el-button>
          <el-button link type="primary" @click="handleDelete(scope.row)" v-hasPermi="['manage:archive:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="archiveRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="件号" prop="jianhao">
          <el-input v-model="form.jianhao" placeholder="请输入件号" />
        </el-form-item>
        <el-form-item label="保密级别" prop="secretLevel">
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="档案位置" prop="location">
          <el-input v-model="form.location" placeholder="请输入档案位置" />
        </el-form-item>
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除档案管理编号为"' + _ids + '"的数据项？').then(function() {
